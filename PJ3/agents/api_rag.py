...
# The embedding model is already cached locally; force offline so loading it does not
# fire (slow, failing) HEAD requests to huggingface.co for update checks. Override by
# exporting HF_HUB_OFFLINE=0 if you actually want it to phone home.
...
class ApiDocRetriever:
    ...
    def _load(self) -> None:
        ...
        if not self._kb_path.exists():
            ui.log(f"[API-RAG] knowledge base not found: {self._kb_path} "
        ...
        ui.log(f"[API-RAG] Loaded {len(self._entries)} API entries.")
        ...
        try:
            ...
            if _EMB_PATH.exists() and _EMB_META.exists() and _EMB_META.read_text().strip() == digest:
                ...
                if self._model is not None:
                    ui.log(f"[API-RAG] Loaded cached embeddings ({self._embeddings.shape[0]}).")
                    return
        except Exception:
            pass
        ...
        if self._model is not None:
            try:
                ...
                ui.log(f"[API-RAG] Built + cached embeddings with {self._model_name}.")
            except Exception as exc:
                ui.log(f"[API-RAG] embedding build failed ({exc}); using keyword fallback.")
                self._model = None
    ...
    def _lazy_model(self):
        try:
            ...
            ui.log(f"[API-RAG] sentence-transformers unavailable ({exc}); keyword fallback.")
            return None
    ...
    # Map a plan component's CONSTRUCTION op to one or more retrieval queries — these
    # target the exact data-API surfaces each recipe touches (so the coder gets the
    # right signatures, e.g. the 2D-fill/extrude props for a flat panel).
    _CONSTRUCTION_QUERY = {
        "extrude": [
            "bpy.types.Curve dimensions 2D fill_mode extrude solid flat panel cyclic bezier_points co",
            "modifier:SOLIDIFY solidify thickness",
        ],
        "revolve": [
            "ScrewModifier lathe revolve profile around axis from_pydata",
        ],
        "sweep": [
            "bpy.types.Curve bevel_depth bevel_object taper path rail Spline BezierSplinePoint co handle_left_type",
        ],
    }
    # For construction=primitive, only the non-trivial primitives are worth docs;
    # cube/cylinder are trivial and add noise.
    _PRIMITIVE_QUERY = {
        "cone": ["primitive_cone_add cone frustum tapered radius1 radius2"],
        "torus": ["primitive_torus_add torus ring major_radius minor_radius"],
        "sphere": ["primitive_uv_sphere_add sphere ball radius"],
    }
    ...
    def retrieve_for_plan(self, plan: dict, top_k_per: int = 1, max_total: int = 6) -> list[str]:
        ...
        if not self._entries:
            return []
        from agents.planner import construction_of

        queries: list[str] = []
        for c in plan.get("components", []):
            constr = construction_of(c)
            if constr == "primitive":
                pt = str(c.get("primitive_type") or c.get("shape") or "").lower()
                queries.extend(self._PRIMITIVE_QUERY.get(pt, []))
            else:
                queries.extend(self._CONSTRUCTION_QUERY.get(constr, []))
