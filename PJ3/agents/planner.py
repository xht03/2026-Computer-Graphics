...
# Back-compat: map the OLD closed "shape" enum onto the new construction ops so
# plans/artifacts produced before the switch still resolve sensibly.
SHAPE_TO_CONSTRUCTION = {
    "cube": "primitive", "cylinder": "primitive", "cone": "primitive",
    "sphere": "primitive", "torus": "primitive", "plane": "primitive",
    "curve": "sweep", "custom": "revolve",
}


def construction_of(component: dict) -> str:
    """Return a component's construction op, deriving it from a legacy 'shape' field
    when 'construction' is absent (old plans)."""
    c = component.get("construction")
    if c:
        return str(c).lower()
    return SHAPE_TO_CONSTRUCTION.get(str(component.get("shape", "")).lower(), "primitive")


def _build_label(component: dict) -> str:
    """Human label of HOW a part is made, e.g. 'cube' or 'extrude'."""
    constr = construction_of(component)
    if constr == "primitive":
        return str(component.get("primitive_type") or component.get("shape") or "cube")
    return constr
...
def summarize_plan_change(old_plan: dict, new_plan: dict) -> str:
    ...
    for n in new_c:
        ...
        diffs = [
            ...
            for f in ("construction", "primitive_type", "profile_desc",
                      "approx_size_m", "count", "position_note")
    ...
    if added:
        ...
        for c in added:
            ln = f"  - {c.get('name')} (x{c.get('count', 1)}, {_build_label(c)}): {c.get('description', '')}"
            if c.get("profile_desc"):
                ln += f", profile: {c['profile_desc']}"
    ...
    def plan_to_coder_description(self, plan: dict) -> str:
        ...
        if plan.get("components"):
            ...
            for c in plan["components"]:
                ...
                profile = c.get("profile_desc", "")
                line = f"  - {c['name']} (x{count}, {how}): {desc}"
                if profile and str(profile).lower() != "null":
