...
class VLMCritic:
    ...
    def critique(self, image_paths: list[str], description: str) -> list[dict]:
        ...
        if not existing:
            ui.log(f"[VLM] No render images found — skipping.")
            return []

        try:
            return self._critique_kimi(existing, description)
        except Exception as e:
            ui.log(f"[VLM] Kimi failed ({e}), falling back to GLM-4V.")
    ...
    def _critique_kimi(self, existing: list[tuple], description: str) -> list[dict]:
        ...
        ui.log(f"[VLM] Kimi found {len(issues)} issue(s).")
        if self.verbose:
            if issues:
                for iss in issues:
    ...
    def _critique_glm4v(self, existing: list[tuple], description: str) -> list[dict]:
        ...
        if self.verbose:
            ...
            print(f"  └─────────────────────────────────────────────────────────────\n")
        issues = self._parse_json(raw)
        ui.log(f"[VLM/GLM-4V] Found {len(issues)} issue(s).")
        if self.verbose:
            if issues:
                for iss in issues:
                    sev = iss.get('severity','?').upper()
