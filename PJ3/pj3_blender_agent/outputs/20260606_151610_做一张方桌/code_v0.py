...
for i in range(4):
    bpy.ops.mesh.primitive_cylinder_add(radius=leg_radius, depth=leg_height, location=(0.45 * math.cos(i * math.pi / 2), 0.45 * math.sin(i * math.pi / 2), -leg_curve))
    leg = bpy.context.active_object
    leg.name = f"leg_{i+1}"
    leg.scale = (leg_radius, leg_radius, leg_height)
    leg.data.materials.append(mat)
    # Apply slight curve to the leg base
# [sanitized]     leg.location.z += leg_curve
# [sanitized]     leg.location.rotate((1, 0, 0), math.pi / 2)