...
for i in range(4):
    bpy.ops.mesh.primitive_cylinder_add(radius=leg_radius, depth=leg_height, location=(0.45 * math.cos(math.radians(90 * i)), 0.45 * math.sin(math.radians(90 * i)), 0.78 - leg_height - leg_curve))
    leg = bpy.context.active_object
    leg.name = f"leg_{i+1}"
    leg.scale = (leg_depth, leg_depth, leg_height)
    leg.data.materials.append(mat)
    # Create a slight curve at the base
# [sanitized]     leg.location.z += leg_curve
    leg.data.vertices[0].co.z += leg_curve
    leg.data.vertices[1].co.z += leg_curve