...
leg_fl = make_box("leg_front_left", 0.1, 0.1, leg_h, -leg_x, leg_y, leg_h/2)

# COMPONENT: leg_front_right
leg_fr = make_box("leg_front_right", 0.1, 0.1, leg_h, leg_x, leg_y, leg_h/2)

# COMPONENT: leg_back_left
leg_bl = make_box("leg_back_left", 0.1, 0.1, leg_h, -leg_x, -leg_y, leg_h/2)

# COMPONENT: leg_back_right
leg_br = make_box("leg_back_right", 0.1, 0.1, leg_h, leg_x, -leg_y, leg_h/2)
...
back_h = 1.1
back_thick = 0.04
back_y = -leg_y  # Align with back legs center
back_panel = make_box("back_panel", seat_w, back_thick, back_h, 0, back_y, back_h/2)

# COMPONENT: armrest_left
# Extends from back panel to front (length 0.48)
arm_len = 0.48
arm_w = 0.1
arm_h = 0.1
arm_z = leg_h + 0.2  # 0.68m from floor
arm_y_start = back_y
arm_y_end = arm_y_start + arm_len  # -0.19 + 0.48 = 0.29
arm_y_center = (arm_y_start + arm_y_end) / 2  # 0.05
arm_x = -(seat_w/2 + arm_w/2)  # -0.325, outside seat
...
curve.bevel_depth = 0.05  # radius for 0.1m diameter

# Configure 3-point curve for forward arch (hat shape)
spline = curve.splines[0]
spline.bezier_points.add(1)  # 3 points total

# Left end
spline.bezier_points[0].co = (-0.275, back_y, back_h, 1)
spline.bezier_points[0].handle_right = (-0.09, back_y, back_h)
spline.bezier_points[0].handle_left_type = 'FREE'
spline.bezier_points[0].handle_right_type = 'FREE'

# Center (forward by 0.1m)
spline.bezier_points[1].co = (0, back_y + 0.1, back_h, 1)
spline.bezier_points[1].handle_left = (-0.09, back_y + 0.1, back_h)
spline.bezier_points[1].handle_right = (0.09, back_y + 0.1, back_h)
spline.bezier_points[1].handle_left_type = 'FREE'
spline.bezier_points[1].handle_right_type = 'FREE'

# Right end
spline.bezier_points[2].co = (0.275, back_y, back_h, 1)
spline.bezier_points[2].handle_left = (0.09, back_y, back_h)
