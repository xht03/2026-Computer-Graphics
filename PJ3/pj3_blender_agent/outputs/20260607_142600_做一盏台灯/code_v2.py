...
ring_heights = [0.14, 0.18, 0.22]
for i, z in enumerate(ring_heights):
    bpy.ops.mesh.primitive_cylinder_add(radius=0.018, depth=0.02, location=(0, 0, z))
    ring = bpy.context.active_object
    ring.name = f"stem_ring_{i+1}"
    ring.data.materials.append(gold_mat)

# COMPONENT: shade
bpy.ops.mesh.primitive_cylinder_add(radius=0.15, depth=0.22, location=(0, 0, 0.55))
...
for i, z in enumerate(trim_heights):
    bpy.ops.mesh.primitive_torus_add(location=(0, 0, z), major_radius=0.15, minor_radius=0.005)
...
pattern_offsets = [(0.155, 0), (0, 0.155), (-0.155, 0), (0, -0.155)]
