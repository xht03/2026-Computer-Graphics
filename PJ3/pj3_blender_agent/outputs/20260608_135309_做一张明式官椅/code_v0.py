...
apron_profile.from_pydata([(0,0,0), (0.02,0,0), (0.02,0,0.10), (0,0,0.10)], [(0,1),(1,2),(2,3),(3,0)], [(0,1,2,3)])
...
# COMPONENT: top_rail (Hat-shaped crest rail)
...
points = [(-0.325, y_back, 1.17), (0, y_back, 1.13), (0.325, y_back, 1.17)]
...
sp = cu.splines.new('BEZIER')
sp.bezier_points.add(2)
# S-curve in X-Z plane at back
pts = [(0, y_back-0.02, 0.52), (0.04, y_back-0.02, 0.82), (-0.04, y_back-0.02, 0.98), (0, y_back-0.02, 1.12)]
...
# COMPONENT: left_armrest (Goose neck curve)
cu = bpy.data.curves.new("left_armrest_curve", type='CURVE')
cu.dimensions = '3D'
cu.bevel_depth = 0.015
sp = cu.splines.new('BEZIER')
sp.bezier_points.add(2)
# Protrudes forward beyond front leg, curves down at front (goose neck)
pts = [(x_left, y_front-0.05, 0.65), (x_left, y_front+0.05, 0.70), (x_left, (y_front+y_back)/2, 0.72), (x_left, y_back, 0.75)]
for bp, co in zip(sp.bezier_points, pts):
    bp.co = co
    bp.handle_left_type = bp.handle_right_type = 'AUTO'
...
sp = cu.splines.new('BEZIER')
sp.bezier_points.add(2)
pts = [(x_right, y_front-0.05, 0.65), (x_right, y_front+0.05, 0.70), (x_right, (y_front+y_back)/2, 0.72), (x_right, y_back, 0.75)]
for bp, co in zip(sp.bezier_points, pts):
    bp.co = co
    bp.handle_left_type = bp.handle_right_type = 'AUTO'
...
# Curved bottom edge, concave upward (traditional shape)
pts = [(x_left, y_front+0.01, 0.38), (x_left/2, y_front+0.01, 0.35), (x_right/2, y_front+0.01, 0.35), (x_right, y_front+0.01, 0.38)]
for bp, co in zip(sp.bezier_points, pts):
    bp.co = co
    bp.handle_left_type = bp.handle_right_type = 'AUTO'
...
sp = cu.splines.new('BEZIER')
sp.bezier_points.add(1)
pts = [(x_left-0.01, y_front, 0.40), (x_left-0.01, (y_front+y_back)/2, 0.36), (x_left-0.01, y_back, 0.40)]
for bp, co in zip(sp.bezier_points, pts):
    bp.co = co
    bp.handle_left_type = bp.handle_right_type = 'AUTO'
...
sp = cu.splines.new('BEZIER')
sp.bezier_points.add(1)
pts = [(x_right+0.01, y_front, 0.40), (x_right+0.01, (y_front+y_back)/2, 0.36), (x_right+0.01, y_back, 0.40)]
for bp, co in zip(sp.bezier_points, pts):
    bp.co = co
    bp.handle_left_type = bp.handle_right_type = 'AUTO'
