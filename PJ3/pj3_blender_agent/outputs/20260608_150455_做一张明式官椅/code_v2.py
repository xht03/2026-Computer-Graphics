...
pts_splat = [(0, -0.20, 0.52), (-0.08, -0.20, 0.73), (0.08, -0.20, 0.94), (0, -0.20, 1.15)]
...
# COMPONENT: side_apron_left
bpy.ops.mesh.primitive_cube_add(size=1, location=(-0.265, 0, 0.47))
side_apron_left = bpy.context.active_object
side_apron_left.name = "side_apron_left"
side_apron_left.scale = (0.02, 0.40, 0.10)
add_bevel(side_apron_left)
side_apron_left.data.materials.append(wood)

# COMPONENT: side_apron_right
bpy.ops.mesh.primitive_cube_add(size=1, location=(0.265, 0, 0.47))
side_apron_right = bpy.context.active_object
side_apron_right.name = "side_apron_right"
side_apron_right.scale = (0.02, 0.40, 0.10)
add_bevel(side_apron_right)
side_apron_right.data.materials.append(wood)
...
pts_al = [(-0.255, -0.20, 0.72), (-0.255, 0.35, 0.72)]
...
pts_ar = [(0.255, -0.20, 0.72), (0.255, 0.35, 0.72)]
...
# COMPONENT: arm_support_left
cu_asl = bpy.data.curves.new("arm_support_left_curve", type='CURVE')
cu_asl.dimensions = '3D'
cu_asl.bevel_depth = 0.02
sp_asl = cu_asl.splines.new('BEZIER')
sp_asl.bezier_points.add(1)
pts_asl = [(-0.255, 0.20, 0.52), (-0.275, 0.25, 0.72)]
for bp, co in zip(sp_asl.bezier_points, pts_asl):
    bp.co = co
    bp.handle_left_type = 'AUTO'
    bp.handle_right_type = 'AUTO'
arm_support_left = bpy.data.objects.new("arm_support_left", cu_asl)
bpy.context.collection.objects.link(arm_support_left)
arm_support_left.data.materials.append(wood)

# COMPONENT: arm_support_right
cu_asr = bpy.data.curves.new("arm_support_right_curve", type='CURVE')
cu_asr.dimensions = '3D'
cu_asr.bevel_depth = 0.02
sp_asr = cu_asr.splines.new('BEZIER')
sp_asr.bezier_points.add(1)
pts_asr = [(0.255, 0.20, 0.52), (0.275, 0.25, 0.72)]
for bp, co in zip(sp_asr.bezier_points, pts_asr):
    bp.co = co
    bp.handle_left_type = 'AUTO'
    bp.handle_right_type = 'AUTO'
arm_support_right = bpy.data.objects.new("arm_support_right", cu_asr)
bpy.context.collection.objects.link(arm_support_right)
arm_support_right.data.materials.append(wood)